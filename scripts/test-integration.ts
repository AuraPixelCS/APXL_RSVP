...
console.log("\nnormalizeRegisterPayload — payment (corporate billing / HRD claims)");
const base = { externalRef: "1", name: "A", email: "a@b.co", ticket: "P1" };
const noPay = normalizeRegisterPayload(base);
check("no payment fields → not pending (original contract)", noPay.ok && !noPay.value.paymentPending);
const unpaid = normalizeRegisterPayload({ ...base, payment_status: "unpaid" });
check("payment_status unpaid → pending", unpaid.ok && unpaid.value.paymentPending);
const awaiting = normalizeRegisterPayload({ ...base, payment_status: "Awaiting Payment" });
check("payment_status 'Awaiting Payment' → pending", awaiting.ok && awaiting.value.paymentPending);
const invoiced = normalizeRegisterPayload({ ...base, paymentStatus: "invoiced" });
check("paymentStatus invoiced → pending", invoiced.ok && invoiced.value.paymentPending);
const paidFalse = normalizeRegisterPayload({ ...base, paid: false });
check("paid: false → pending", paidFalse.ok && paidFalse.value.paymentPending);
const paidTrue = normalizeRegisterPayload({ ...base, paid: true, payment_status: "unpaid" });
check("paid: true beats an unpaid status", paidTrue.ok && !paidTrue.value.paymentPending);
const paidStatus = normalizeRegisterPayload({ ...base, payment_status: "paid" });
check("payment_status paid → not pending", paidStatus.ok && !paidStatus.value.paymentPending);
const junkStatus = normalizeRegisterPayload({ ...base, payment_status: "maybe-later" });
check("unrecognised payment_status rejected", !junkStatus.ok && (junkStatus as any).field === "payment_status");
const hrd = normalizeRegisterPayload({ ...base, payment_status: "unpaid", payment_method: "SBL-KHAS claim (HRD Corp)" });
check("HRD method canonicalised", hrd.ok && hrd.value.paymentMethod === "hrd_claim");
const corp = normalizeRegisterPayload({ ...base, payment_status: "unpaid", payment_method: "Corporate billing (invoice)" });
check("corporate method canonicalised", corp.ok && corp.value.paymentMethod === "corporate_billing");
const self = normalizeRegisterPayload({ ...base, payment_method: "Self-funded (pay now)" });
check("self-funded method canonicalised, not pending", self.ok && self.value.paymentMethod === "self_funded" && !self.value.paymentPending);
const methodOnly = normalizeRegisterPayload({ ...base, payment_method: "corporate_billing" });
check("method alone never flips to pending", methodOnly.ok && !methodOnly.value.paymentPending);
